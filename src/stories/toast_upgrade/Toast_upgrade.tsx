...
import { updateMailboxAlarm } from "common/mailbox/mail_func";
...
export default function RealTimeBroadUI() {
  ...
  useEffect(() => {
    return () => {
      if (checkChatting !== "chatting") {
        updateMailboxAlarm(mailboxAction);
      }
    };
  }, []);
  ...
  .wrapper {
    ...
    &.mailbox {
      ...
      .close {
        ...
        height: 36px;
        margin-left: auto;
        background: url("http://image.dalbitlive.com/svg/ico_close_b_m.svg");
      }
    }
    .close {
      width: 36px;
